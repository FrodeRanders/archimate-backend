package org.gautelis.archimesh.plugin.ui;
...
/**
 * Dialog for opening a Archimesh-backed model directly from server.
 */
public class OpenServerModelDialog extends TitleAreaDialog {
    ...
    public void create() {
        super.create();
        setTitle("Open Archimesh Model From Server");
        setMessage("Open an in-memory model from the Archimesh server and connect immediately.");
    ...
    protected void okPressed() {
        ...
        if(sessionId.isEmpty()) {
            sessionId = "archimesh-open-" + UUID.randomUUID();
        }
    ...
    private void updateModelRefDisplay() {
        ...
        modelRefModeReadOnlyText.setText("HEAD".equalsIgnoreCase(ref) ? "Writable live Archimesh tip" : "Read-only tagged snapshot");
    }
    ...
    private void updateAuthHint() {
        ...
        String token = trimOrEmpty(authTokenText.getText());
        authHintLabel.setText(ArchimeshAuthHints.describePreflightAuthHint(!token.isEmpty()));
        if(authTokenExpiryLabel != null && !authTokenExpiryLabel.isDisposed()) {
            authTokenExpiryLabel.setText(ArchimeshAuthHints.describeTokenExpiry(token));
        }
        if(authTokenIdentityLabel != null && !authTokenIdentityLabel.isDisposed()) {
            authTokenIdentityLabel.setText(ArchimeshAuthHints.describeTokenIdentity(token));
        }
    ...
    private void copyTokenPreview() {
        ...
        String token = trimOrEmpty(authTokenText.getText());
        String payload = "tokenStatus=" + ArchimeshAuthHints.describeTokenExpiry(token)
                + System.lineSeparator()
                + "tokenIdentity=" + ArchimeshAuthHints.describeTokenIdentity(token);
