...
import org.gautelis.archimesh.endpoint.ArchimeshEndpoint;
...
public class InMemorySessionRegistry implements SessionRegistry {
    ...
    private AdminActiveSession toAdminActiveSession(Session session) {
        ...
        Object rawRoles = session.getUserProperties().get(ArchimeshEndpoint.AUTH_SUBJECT_ROLES_KEY);
        Set<String> roles = rawRoles instanceof Set<?> values
                ...
                : Set.of();
        String userId = stringProp(session, ArchimeshEndpoint.AUTH_SUBJECT_USER_ID_KEY);
        String ref = stringProp(session, ArchimeshEndpoint.AUTH_SUBJECT_REF_KEY);
        boolean writable = Boolean.parseBoolean(stringProp(session, ArchimeshEndpoint.AUTH_SUBJECT_WRITABLE_KEY));
