...
class ArchimeshEndpointConfiguratorTest {
    ...
    void capturesPrincipalAndConfiguredRolesFromHandshake() {
        ...
        try {
        ArchimeshEndpointConfigurator configurator = new ArchimeshEndpointConfigurator();
        ...
        Assertions.assertEquals("oidc-ws-user",
                config.getUserProperties().get(ArchimeshEndpointConfigurator.HANDSHAKE_PRINCIPAL_NAME_KEY));
        Assertions.assertEquals(Set.of("admin", "model_writer"),
                config.getUserProperties().get(ArchimeshEndpointConfigurator.HANDSHAKE_ROLES_KEY));
        } finally {
