# Archimesh Centralization Plan

## Goal
Make the Archimesh server the authoritative source of truth for models, with local files acting as cache/recovery artifacts only.
...
- Persist server-backed models to deterministic cache path:
  - `~/Archi/archimesh-cache/<modelId>.archimate`
...
- Objective: minimize/avoid save prompts for Archimesh-backed sessions on close.
...
- Added websocket close handling to preserve Archimesh model context while offline.
...
- Added durable outbox persistence:
  - queued ops are persisted per model under Archimesh cache directory.
